import org.jetbrains.kotlin.gradle.dsl.JvmTarget

plugins {
    alias(libs.plugins.kotlin.multiplatform)
    alias(libs.plugins.android.library)
    alias(libs.plugins.kotlin.compose)
    alias(libs.plugins.compose.multiplatform)
    alias(libs.plugins.serialization)
}

kotlin {
    androidTarget {
        compilerOptions { jvmTarget.set(JvmTarget.JVM_17) }
    }
    iosX64()
    iosArm64()
    iosSimulatorArm64()

    sourceSets {
        commonMain.dependencies {
            implementation(libs.komposed)
            implementation(project(":app:core:navigation"))
            implementation(project(":app:core:middleware"))
            implementation(project(":app:checkout:delivery"))
            implementation(project(":app:checkout:bill"))
            implementation(project(":app:checkout:placeOrder"))
            implementation(compose.material3)
            implementation(compose.materialIconsExtended)
            implementation(libs.serialization)
            implementation(libs.androidx.navigation.compose.kmp)
            implementation(libs.koin.core)
            implementation(libs.koin.core.viewmodel)
        }
        androidUnitTest.dependencies {
            implementation(libs.junit)
            implementation(libs.kotlinx.coroutines.test)
            implementation(libs.komposed.testing)
        }
    }
}

android {
    namespace = "io.github.atwa.komposed.app.checkout"
    compileSdk = 36
    defaultConfig {
        minSdk = 24
        consumerProguardFiles("consumer-rules.pro")
    }
    buildFeatures { compose = true }
    compileOptions {
        sourceCompatibility = JavaVersion.VERSION_17
        targetCompatibility = JavaVersion.VERSION_17
    }
    sourceSets["main"].manifest.srcFile("src/androidMain/AndroidManifest.xml")
}
